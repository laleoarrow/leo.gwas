...
This document defines practical development rules for `leo.gwas`. The
goal is to keep code, docs, and CI behavior consistent and reproducible.
...
- `.github/workflows/`: CI workflows (`R-CMD-check`, `pkgdown`,
  `test-coverage`).
...
- For progress/status messages, prefer
  [`leo.basic::leo_log()`](https://rdrr.io/pkg/leo.basic/man/leo_log.html)
...
- Every exported topic must appear in `_pkgdown.yml`
  `reference.contents`, or be marked with `@keywords internal`.
...
- For GitHub-only dependencies, document rationale in `DESCRIPTION`
  `Remotes`.
...
If step 4 is too heavy locally, at least ensure steps 1-3 pass before
push.
